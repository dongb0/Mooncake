...
DEFINE_uint64(block_iteration, 1, "number of iterations of the block");
DEFINE_bool(auto_discovery, false, "Enable auto discovery");
DEFINE_uint64(npu_id, 65536, "The device logic and phy ID of this machine");
...
DEFINE_string(device_name, "hns_2",
...
int allocateDevMem(void *&devAddr, size_t size) {
    ...
    for (size_t i = 0; i < size; i += sizeof(uint32_t)) {
        *(uint32_t *)((char *)host_addr + i) = 0x12345678 + i;
    }
...
int target() {
    ...
    // release resource
    aclrtFreeHost(tmp_hostAddr);
    for (uint32_t i = 0; i < FLAGS_block_iteration; i++) {
        aclrtFreeHost(g_addr[i]);
    }
...
int main(int argc, char **argv) {
    ...
    if (FLAGS_npu_id != 65536) {
        g_deviceLogicId = FLAGS_npu_id;
        g_devicePhyId = FLAGS_npu_id;
    } else {
